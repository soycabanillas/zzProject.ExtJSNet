...
namespace zzProject.ExtJSNet.ExtJS.Classes.Ext.data
{
    public abstract class TreeODataStoreAbstract : TreeStoreBase
    {
        ...
        {
            attributes.Add("proxy", new zzProject.ExtJSNet.ExtJS.Classes.Ext.data.proxy.ODataProxy(this._url));
            base.Serialize(writer, value, serializer);
        }

        public TreeODataStoreAbstract(NodeInterface root, string url) : base(root)
        {
            this._url = url;
        }

        public TreeODataStoreAbstract(string modelName, NodeInterface root, string url)
            : base(modelName, root)
        {
            this._url = url;
        }

        public TreeODataStoreAbstract(Model model, NodeInterface root, string url)
            : base(model, root)
        {
            this._url = url;
        }
    ...
    public class TreeODataStore : TreeODataStoreAbstract
    {
        ...
        public TreeODataStore(NodeInterface root, string url)
            : base(root, url)
        {
        }

        public TreeODataStore(string modelName, NodeInterface root, string url)
            : base(modelName, root, url)
        {
        }

        public TreeODataStore(Model model, NodeInterface root, string url)
            : base(model, root, url)
        {
        }
    }
